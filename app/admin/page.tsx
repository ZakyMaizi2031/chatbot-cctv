...
export default async function AdminPanel(props: { searchParams: Promise<{ tab?: string, page?: string, date?: string, search?: string, history?: string, detail?: string }> }) {
  ...
  // Device Detail Variables
  let detailLogs: any[] = [];
  let detailDeviceData: any = null;
  let totalDetailItems = 0;
  let detailOnlineCount = 0;
  let detailOfflineCount = 0;
  const detailId = searchParams.detail || null;

  if (detailId) {
    const detailCountRes = await sql`SELECT COUNT(*) as count FROM notification_logs WHERE device_id = ${detailId}`;
    totalDetailItems = parseInt(detailCountRes[0].count);
                        ...
                        {devices.map((dev, index) => {
                          ...
                          return (
                            <tr key={dev.device_id} className="hover:bg-blue-50/40 transition-colors group">
                              <td className="py-2.5 px-4 text-center text-slate-400 font-medium text-sm">{offset + index + 1}</td>
                              <td className="py-2.5 px-4">
                                <Link href={`?tab=${tab}&detail=${dev.device_id}`} className="font-semibold text-slate-800 text-sm group-hover:text-blue-700 transition-colors underline decoration-transparent hover:decoration-blue-700">
                                  {dev.device_name}
                                </Link>
                              </td>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Link href={`?tab=${tab}`} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-sm font-semibold transition-colors shadow-sm">
